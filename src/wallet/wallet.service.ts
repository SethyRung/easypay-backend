...
export class WalletService {
  ...
  constructor(
    ...
    this.topUpMaxPerTxMinor = Number(
      this.configService.get('TOPUP_MAX_PER_TX_MINOR') || '100000',
    );
    this.topUpMaxDailyMinor = Number(
      this.configService.get('TOPUP_MAX_DAILY_MINOR') || '500000',
    );
  }

  async getBalance(userId: string) {
    const wallet =
      await this.walletRepository.getOrCreateWalletByUserId(userId);
  ...
  async getTransactions(userId: string, query: TransactionsQueryDto) {
    ...
    const { transactions, total } =
      await this.walletRepository.getTransactionHistory(
        wallet.id,
        query.limit || 10,
        query.offset || 0,
      );
  ...
  async withdrawWallet(userId: string, dto: any): Promise<any> {
    ...
    const result = await this.walletRepository.withdrawWallet(
      userId,
      amountMinor,
      dto.note,
    );
