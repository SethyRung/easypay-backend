...
const SET_AUTH_TOKEN_HEADER = {
  description:
    'Bearer token. Send as `Authorization: Bearer <token>` on subsequent requests.',
  schema: { type: 'string' },
};
...
export function appendAuthPaths(doc: OpenAPIObject): void {
  ...
  doc.tags.push({
    name: 'auth',
    description:
      'Better-auth sessions — sign-up, sign-in, session, password reset',
  });
  ...
  Object.assign(doc.paths, {
    '/api/auth/ok': {
      get: {
        ...
        description:
          'Returns `{ ok: true }` if BA is reachable. No auth required.',
    ...
    '/api/auth/sign-up/email': {
      post: {
        ...
        responses: {
          ...
          '422': {
            description:
              'Validation error (missing field, weak password, duplicate email/phone)',
          },
        },
      },
    },
    '/api/auth/sign-in/email': {
      post: {
        ...
        description:
          'Issues a session token in the `Set-Auth-Token` response header.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
