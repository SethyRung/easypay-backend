...
export class TransfersService {
  ...
  constructor(
    ...
    this.feeMinor = Number(
      this.configService.get('TRANSFER_FEE_MINOR') || '200',
    );
  }
  ...
  async createTransfer(
    ...
    const existingTransfer =
      await this.transfersRepository.findTransferByIdempotency(
        senderUserId,
        dto.idempotencyKey,
      );
    if (existingTransfer) {
    ...
    const recipient = await this.transfersRepository.findByPhone(
      dto.recipientPhone,
    );
    if (!recipient) {
    ...
    return this.db.transaction(async (db) => {
      ...
      const senderDebitBalanceAfter =
        senderWallet.balanceMinor - amountMinor - this.feeMinor;
      ...
      const recipientCreditBalanceAfter =
      ...
      await db
        .update(walletAccounts)
        .set({
          balanceMinor: recipientCreditBalanceAfter,
          updatedAt: new Date(),
        })
  ...
  async getTransfer(transferId: string): Promise<TransferReceiptDto> {
    const transfer =
      await this.transfersRepository.findTransferById(transferId);
    if (!transfer) {
    ...
    const recipientCredit = senderDebitEntries.find(
