...
describe("BridgeIssueService.issue", () => {
  ...
  beforeEach(() => {
    bridgeUserRepository = {
      findById: jest.fn().mockResolvedValue({ id: "user-1", email: "user-1@example.com" }),
  ...
  it("happy path: 200 envelope with ticket → returns { ticket }", async () => {
    httpService.post.mockReturnValue(
      of(
        makeResponse(200, { status: { code: ApiResponseCode.Success }, data: { ticket: TICKET } }),
      ),
    );
  ...
  it("local user not found → throws NotFoundException without calling glitch", async () => {
    ...
    await expect(service.issue("missing")).rejects.toBeInstanceOf(NotFoundException);
  ...
  it("401 from glitch → throws UnauthorizedException", async () => {
    ...
    await expect(service.issue("user-1")).rejects.toBeInstanceOf(UnauthorizedException);
  });
  ...
  it("404 from glitch → throws NotFoundException", async () => {
    ...
    await expect(service.issue("user-1")).rejects.toBeInstanceOf(NotFoundException);
    await expect(service.issue("user-1")).rejects.toThrow("User not provisioned on bridge partner");
  });
  ...
  it("5xx from glitch → throws ServiceUnavailableException", async () => {
    ...
    await expect(service.issue("user-1")).rejects.toBeInstanceOf(ServiceUnavailableException);
    await expect(service.issue("user-1")).rejects.toThrow("Bridge partner unavailable");
  });
  ...
  it("4xx (non-401, non-404) → throws ServiceUnavailableException", async () => {
    httpService.post.mockReturnValue(
      of(makeResponse(422, { status: { code: ApiResponseCode.InvalidRequest } })),
    );
  ...
  it("200 envelope without data.ticket → throws ServiceUnavailableException", async () => {
    httpService.post.mockReturnValue(
      of(makeResponse(200, { status: { code: ApiResponseCode.Success }, data: {} })),
    );

    await expect(service.issue("user-1")).rejects.toBeInstanceOf(ServiceUnavailableException);
    await expect(service.issue("user-1")).rejects.toThrow("Bridge partner response missing ticket");
  });

  it("200 envelope with empty-string ticket → throws ServiceUnavailableException", async () => {
    httpService.post.mockReturnValue(
      of(makeResponse(200, { status: { code: ApiResponseCode.Success }, data: { ticket: "" } })),
    );
  ...
  it("200 + INVALID_REQUEST envelope with message → surfaces glitch's cause in a 503", async () => {
    httpService.post.mockReturnValue(
      of(
        makeResponse(200, {
          status: { code: ApiResponseCode.InvalidRequest, message: "hash mismatch" },
        }) as AxiosResponse,
      ),
    );

    await expect(service.issue("user-1")).rejects.toBeInstanceOf(ServiceUnavailableException);
    await expect(service.issue("user-1")).rejects.toThrow("Bridge partner rejected request");
  ...
  it("200 + INVALID_REQUEST envelope without message → 503 with default text", async () => {
    httpService.post.mockReturnValue(
      of(makeResponse(200, { status: { code: ApiResponseCode.InvalidRequest } })),
    );
  ...
  it("200 + INTERNAL_ERROR envelope with message → surfaces glitch's cause in a 503", async () => {
    httpService.post.mockReturnValue(
      of(
        makeResponse(200, {
          status: { code: ApiResponseCode.InternalError, message: "user upsert failed" },
        }) as AxiosResponse,
      ),
    );

    await expect(service.issue("user-1")).rejects.toBeInstanceOf(ServiceUnavailableException);
    await expect(service.issue("user-1")).rejects.toThrow("Bridge partner internal error");
  ...
  it("200 + INTERNAL_ERROR envelope without message → 503 with default text", async () => {
    httpService.post.mockReturnValue(
      of(makeResponse(200, { status: { code: ApiResponseCode.InternalError } })),
    );
